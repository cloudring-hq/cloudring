...
PROJECT_NAME="${PROJECT_NAME:-tenant-c}"
LOCK_NAME="${LOCK_NAME:-project-quota-admission-${PROJECT_NAME}}"
TENANT_NAMESPACE="${TENANT_NAMESPACE:-${PROJECT_NAME}}"
CLAIM_NAME="${CLAIM_NAME:-project-quota-lock-vm}"
VM_NAME="claim-${CLAIM_NAME}"
RESERVATION_NAME="vmc-${TENANT_NAMESPACE}-${CLAIM_NAME}"
LOCK_HOLDER="verify-project-quota-admission-lock"
original_project_quotas=""
original_resource_quota_hard=""

restore_quota() {
  if [[ -n "${original_project_quotas}" ]]; then
    ${KUBECTL} patch project "${PROJECT_NAME}" --type merge \
...
wait_lock_free() {
  ...
  echo "project quota admission lock ${LOCK_NAMESPACE}/${LOCK_NAME} remained held by ${holder}" >&2
...
original_project_quotas="$(
  ${KUBECTL} get project "${PROJECT_NAME}" -o json | python3 -c '
import json
import sys
...
echo "== admission is blocked while synthetic project quota lock is held =="
wait_jsonpath "PendingAdmission" '{.status.phase}' \
  ${KUBECTL} -n "${TENANT_NAMESPACE}" get virtualmachineclaim "${CLAIM_NAME}"
wait_jsonpath "ProjectQuotaAdmissionLocked" '{.status.admission.reason}' \
  ${KUBECTL} -n "${TENANT_NAMESPACE}" get virtualmachineclaim "${CLAIM_NAME}"
if ${KUBECTL} -n "${TENANT_NAMESPACE}" get vm "${VM_NAME}" >/dev/null 2>&1; then
  echo "backing VM ${TENANT_NAMESPACE}/${VM_NAME} was created while project quota admission lock was held" >&2
  exit 1
fi
if ${KUBECTL} get capacityreservation "${RESERVATION_NAME}" >/dev/null 2>&1; then
  echo "capacity reservation ${RESERVATION_NAME} was created while project quota admission lock was held" >&2
  exit 1
fi
...
echo "project quota admission lock verification passed"
